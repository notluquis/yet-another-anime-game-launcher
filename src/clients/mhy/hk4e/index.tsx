...
import { Sophon, createSophonRetryWithProgress } from "@sophon";
...
export async function createHK4EChannelClient({
	...
	const { pid: spid } = await spawn(
		[resolve("./sidecar/sophon_server/sophon-server")],
		{
	...
	// Create a promise that will be resolved when Sophon is connected
	let _sophonResolve: (client: Sophon) => void;
	const sophonPromise = new Promise<Sophon>((resolve) => {
		_sophonResolve = resolve;
	});
	const getSophon = () => sophonPromise;

	// Fetch game info will be done after init connects to Sophon
	let LATEST_GAME_VERSION: string = "0.0.0";
	let UPDATABLE_VERSIONS: string[] = [];
	let PRE_DOWNLOAD_VERSION: string = "0.0.0";
	let PRE_DOWNLOAD_AVAILABLE: boolean = false;
	let INSTALL_SIZE_BYTES: number = 0;
	...
	return {
		...
		async *init(config: Config) {
			yield ["setStateText", "INITIALIZING_SOPHON_START"];

			// Connect to Sophon with progress updates
			let sophonClient: Sophon | null = null;
			try {
				for await (const progress of createSophonRetryWithProgress(
					sophon_host,
					sophon_port,
				)) {
					if (progress[0] === "sophonClient") {
						sophonClient = progress[1];
					} else {
						yield progress;
					}
				}
				...
				throw new Error(
					`Failed to connect to Sophon: ${error instanceof Error ? error.message : String(error)}`,
				);
			}

			if (!sophonClient) {
				throw new Error("Sophon client not initialized");
			}

			// Resolve the Sophon promise so all methods can use it
			_sophonResolve(sophonClient);

			yield ["setStateText", "INITIALIZING_SOPHON_COMPLETE"];
			yield ["setStateText", "INITIALIZING_FETCHING_GAME_INFO"];

			// Fetch game info from Sophon
			const gameInfo = await sophonClient.getLatestOnlineGameInfo(
				releaseType,
				"hk4e",
			);
			log(`Game info: ${JSON.stringify(gameInfo)}`);
			LATEST_GAME_VERSION = gameInfo.version;
			UPDATABLE_VERSIONS = gameInfo.updatable_versions;
			PRE_DOWNLOAD_VERSION = gameInfo.pre_download_version || "0.0.0";
			PRE_DOWNLOAD_AVAILABLE = gameInfo.pre_download;
			INSTALL_SIZE_BYTES = gameInfo.install_size;

			yield ["setStateText", "INITIALIZING_CHECKING_INSTALLATION"];
			...
			yield ["setStateText", "INITIALIZING_COMPLETE"];
		},
...
async function checkGameState(locale: Locale, server: Server) {
	...
	if (await isExternalDrive(gameDir)) {
		...
		if (!isAccessible) {
			await log(
				`Game directory on external drive but not accessible: ${gameDir}`,
			);
			return {
				gameInstalled: false,
			} as const;
